...
# Usage:
#	From this directory, run $ sudo docker build -t rpi_minecraft_game_server:v1.0 -f RpiMinecraftGameServer.Dockerfile .
...
>> server.properties
RUN ifconfig
RUN echo 'Container is ready, run it with the command $ sudo docker run -p 25565:25565 -it rpi_minecraft_game_server:v1.0'
